...
```{note}
Results exhibiting **numerical instability** or **non-realistic dynamics** (see [Numerical Implementation](./numerical.implementation.md) and [Writing Your Controller](./writing_controller.md)) may be subject to **manual** review. If the reported performance is determined to arise from **numerical artifacts** rather than consistent dynamic behavior, the corresponding **results** will be **excluded** from the official **evaluation**, ensuring fairness and compliance with the physical modeling assumptions and numerical limits of the WAPPAC platform.
```
